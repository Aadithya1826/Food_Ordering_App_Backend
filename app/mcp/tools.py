...
def build_customer_tool_prompt(
    ...
    if page == "/":
        ...
        lines.extend([
            "SELECT_ORDER_TYPE:",
            "  → Ask: 'Would you prefer Dine-In or Takeaway?'",
            ...
            "      - If table_number is ALREADY KNOWN: navigate to dine-in, advance to MENU_BROWSE. ui_actions: [{ action: set_order_type, type: dine-in }, { action: navigate, page: dine-in }, { action: set_flow_stage, stage: MENU_BROWSE }]",
            "  → If 'Takeaway' / 'Take Away' / similar: emit set_order_type:takeaway, navigate, advance to MENU_BROWSE.",
            "  → For Takeaway: ui_actions: [{ action: set_order_type, type: takeaway }, { action: navigate, page: take-away }, { action: set_flow_stage, stage: MENU_BROWSE }]",
            "",
            "COLLECT_TABLE (Dine-In only, if table_number is missing):",
            "  → Ask: 'Which table are you sitting at?'",
            "  → When customer gives table number: emit set_table_number, navigate to dine-in, advance to MENU_BROWSE.",
            "  → ui_actions: [{ action: set_table_number, table: <num> }, { action: navigate, page: dine-in }, { action: set_flow_stage, stage: MENU_BROWSE }]",
            "",
